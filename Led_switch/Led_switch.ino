...
/*
 * 这属于html内容了
 * \"中的\是为了告诉编译器"不是最后结束的那个"，\是必须加的，详细解释见B站太极创客esp8266教程3-2的37分左右
<form action=\"/LED\" method=\"POST\">开始
<input type=\"submit\" value=\"Toggle LED\">//操作内容
</form>"//结束
*/
}
...
void handleLED() {                          
  digitalWrite(LED_BUILTIN,!digitalRead(LED_BUILTIN));// 改变LED的点亮或者熄灭状态，先读后写，作用是取反
  esp8266_server.sendHeader("Location","/");          // 跳转回页面根目录
  esp8266_server.send(303);                           // 发送Http相应代码303 跳转  
  //此处最后两条语句要结合起来看，即跳转到首页
  //整个函数的作用是将led对应端口电平取反之后再跳转到服务器的首页
}
