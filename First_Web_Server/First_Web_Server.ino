...
本工程详细解释见：http://www.taichi-maker.com/homepage/esp8266-nodemcu-iot/iot-c/esp8266-nodemcu-web-server/web-server/
...
void setup(void){
  ...
  wifiMulti.addAP("511", "511511511");  // 这三条语句通过调用函数addAP来记录3个不同的WiFi网络信息。
...
//--------"启动网络服务功能"程序部分开始-------- //  此部分为程序为本示例程序重点1
  ...
  esp8266_server.on("/", handleRoot);       //  访问根路径时，执行函数handleRoot
  esp8266_server.onNotFound(handleNotFound);   //请求资源没找到时，执行函数 handleNotFound    
//--------"启动网络服务功能"程序部分结束--------
...
void loop(void){
  esp8266_server.handleClient();     // 处理http服务器访问，要不断调用这条函数，这样esp8266才能处理用户访问
}
...
void handleRoot() {   //处理网站根目录“/”的访问请求 
  ...
  //即esp8266作为服务器向浏览器发送纯文本信息：“Hello from ESP8266”
}
